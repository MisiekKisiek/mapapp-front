...
const MarkerList = ({ markersAll, handleMarkerListActiveItem, removeMarker }) => {
    ...
    const renderMarkerItems = (filter) => {
        ...
        const markers = markersAll.filter(e => {
            const allInfo = `${e.name ? e.name : ""}${e.place ? e.place : ""}${e.description ? e.description : ""}`;
            console.log(allInfo)
            ...
            <div className="marker__wrap">
                <form className="marker__form-filter">
                    <div className="marker__filter">
                        <input type="text" placeholder="Search..." value={filterValue} onChange={handleFilter} />
                        <button onClick={handleRemoveFilter} className='marker__form-clear'>
                            <i class="fas fa-times-circle"></i>
                        </button>
                        <button type="submit" className='marker__form-submit' onClick={handleFilter}>
                            <i className="fas fa-search"></i>
                        </button>
                    </div>
                </form>
                <ul className="marker__list">
                    {renderMarkerItems(filter)}
                </ul>
            </div>
        </aside>
    </>);
