...
const MarkerListItem = ({ handleMarkerListActiveItem, removeMarker, marker, first, id }) => {
    ...
    return (<>
        <li className={`marker__item ${first ? 'marker__item--active' : ''}`} data-markerid={id} onClick={(e) => { handleMarkerListActiveItem(e) }}>
            ...
            <button className="marker__remove-btn" onClick={removeMarkerFunc}><i class="fas fa-times"></i></button>
            <button className="marker__edit-btn"><i class="fas fa-pen"></i></button>
        </li>
    </>);
}
